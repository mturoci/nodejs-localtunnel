...
class TunnelAgent extends Agent {
  constructor(options = {}) {
    ...
    const { maxTcpSockets, sslCert, sslKey } = options
    ...
    this.maxTcpSockets = maxTcpSockets || DEFAULT_MAX_SOCKETS

    this.server = options.sslCert && options.sslKey
      ? tls.createServer({ cert: fs.readFileSync(sslCert), key: fs.readFileSync(sslKey), }, this._onConnection)
      : net.createServer({}, this._onConnection)
